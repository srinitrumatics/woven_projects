...
async function testBulkUpdateSimple() {
    console.log('Testing bulk update with NO change detection...\n');
    ...
    try {
        ...
        console.log(`Testing with ${sfids.length} products`);
        ...
        console.log('Queue entries found:', queueCheck.rows.length);
        console.log('Expected:', sfids.length);

        if (queueCheck.rows.length === sfids.length) {
            console.log('\n✅ SUCCESS! All bulk updates were queued!');
            console.table(queueCheck.rows);
        } else if (queueCheck.rows.length === 0) {
            console.log('\n❌ FAILED! No entries were queued.');
            console.log('\nPlease run this SQL to apply the fix:');
            console.log('psql $DATABASE_URL < fix-trigger-no-change-detection.sql');
        } else {
            console.log('\n⚠️  PARTIAL! Only some entries were queued.');
            console.table(queueCheck.rows);
        }
