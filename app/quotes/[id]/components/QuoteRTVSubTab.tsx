        ...
        <div className="overflow-x-auto py-2">
            <table className="w-full whitespace-nowrap">
                <thead className="bg-primary-light dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        <SortableHeader label="RTV Number" field="rtvNumber" sortConfig={sortConfig} requestSort={requestSort} width={widths.rtvNumber} onResize={onResize} align="left" className="sticky left-0 bg-primary-light dark:bg-gray-900 z-10" />
                        ...
                        <SortableHeader label="Reason" field="reason" sortConfig={sortConfig} requestSort={requestSort} width={widths.reason} onResize={onResize} align="left" />
                    </tr>
                </thead>
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {rtvs.length === 0 ? (
                        <tr>
                            <td colSpan={6} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No RTVs found</td>
                        </tr>
                    ) : (
                        rtvs.map((rtv) => (
                            <tr key={rtv.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td className="px-3 py-2 text-sm text-gray-900 dark:text-white" style={{ width: widths.vendor }}>{rtv.vendor}</td>
                                <td className="px-3 py-2 text-sm text-gray-900 dark:text-white" style={{ width: widths.date }}>{rtv.date}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900 dark:text-white" style={{ width: widths.totalAmount }}>${rtv.totalAmount.toFixed(2)}</td>
                                <td className="px-3 py-2 text-sm text-gray-900 dark:text-white" style={{ width: widths.reason }}>{rtv.reason}</td>
                            </tr>
                        ))
                    )}
                </tbody>
