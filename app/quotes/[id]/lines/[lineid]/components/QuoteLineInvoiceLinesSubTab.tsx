            ...
            <div className="overflow-x-auto">
                {data.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400 min-w-0">
                        ...
                        <p className="text-sm truncate">There are no invoices associated with this quote line.</p>
                    </div>
                ) : (
                            ...
                            {paginatedData.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td className="px-3 py-2 text-sm text-gray-900 dark:text-white truncate" style={{ width: widths.invoiceName }}>
                                        {item.invoiceId ? (
                                            <Link href={`/invoices/${item.invoiceId}`} className="text-primary hover:underline font-medium">
                                                {item.invoiceName}
                                            </Link>
                                        ) : item.invoiceName}
                                    </td>
                                    ...
                                    <td className="px-3 py-2 text-sm text-gray-900 dark:text-white truncate" style={{ width: widths.invoiceQty }}>{item.invoiceQty}</td>
