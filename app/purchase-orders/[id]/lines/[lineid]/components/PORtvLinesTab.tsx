...
export default function PORtvLinesTab({ lines }: PORtvLinesTabProps) {
    ...
    if (lines.length === 0) {
        return (
            <TableEmptyState message="No RTV lines found" description="There are no RTV lines associated with this record." />
        );
    }
            ...
            <div className="flex-1 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
                <Table className="border-separate border-spacing-0">
                    <THead className="sticky top-0 z-20">
                        <tr>
                            <SortableHeader label="RTV Line" field="Name" sortConfig={sortConfig} requestSort={requestSort} width={columnWidths.name} onResize={handleResize} className="sticky left-0 bg-primary-light dark:bg-gray-900 z-30" />
                    ...
                    <TBody>
                        {paginatedData.map((line) => (
                            <Tr key={line.Id} className="transition-colors group">
                                ...
                                <Td className="truncate" title={line.RTV_Name || '-'}>
                                ...
                                <Td className="truncate" title={line.Proposed_Product_Name || '-'}>
                                    {line.Proposed_Product__c ? (
                                        !isManufacturer ? (
                                            <Link href={`/proposals/${line.proposal__c}/lines/${line.Proposed_Product__c}`} target="_blank" className="text-primary hover:underline font-semibold">
                                ...
                                <Td className="text-left truncate" title={String(line.Return_Qty__c || 0)}>{line.Return_Qty__c || 0}</Td>
