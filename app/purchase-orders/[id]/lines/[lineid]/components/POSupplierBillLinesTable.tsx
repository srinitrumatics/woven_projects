                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {paginatedData.map((line) => (
                            <tr key={line.Id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                ...
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white"><div className="truncate" title={line.Supplier_Bill_Name}>{line.Supplier_Bill_Name || ' '}</div></td>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white font-semibold text-left">{formatCurrency(line.BillAmount__c || 0)}</td>
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white text-left">{formatCurrency(line.Shipping_Charges__c || 0)}</td>
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white font-semibold text-left">{formatCurrency(line.Total_Bill_Amount__c || 0)}</td>
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{line.Goods_Receipt_Date__c ? formatDate(line.Goods_Receipt_Date__c, 'numeric-dash') : ' '}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
