        ...
        <div className="overflow-x-auto">
            <table className="w-full table-fixed">
                <thead className="bg-primary-light dark:bg-gray-900">
                    <tr>
                        <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 dark:text-white" style={{ width: widths.image || 100 }}>&nbsp;</th>
                        ...
                        <SortableHeader label="Total Order Qty" field="quantity" sortConfig={sortConfig} requestSort={requestSort} width={widths.quantity} onResize={onResize} align="right" />
                        <SortableHeader label="Total Price" field="subtotal" sortConfig={sortConfig} requestSort={requestSort} width={widths.subtotal} onResize={onResize} align="right" />
                        ...
                        <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 dark:text-white" style={{ width: widths.actions || 100 }}>Action</th>
                    </tr>
                </thead>
                    ...
                    {products.length === 0 ? (
                        ...
                        products.map((product) => (
                            <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <td className="px-4 py-3" style={{ width: widths.image || 100 }}>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white font-medium text-left" style={{ width: widths.productName }}>
                                    ...
                                    <div className="text-sm font-mono text-gray-500 dark:text-gray-400 mt-1 line-clamp-2 text-left">{product.description}</div>
                                </td>
                                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white text-left" style={{ width: widths.manufacturer }}><div className="line-clamp-2">{product.manufacturer}</div></td>
                                <td className="px-4 py-3 text-right" style={{ width: widths.productFamily }}>
                                    <span className="inline-block px-2 py-1 text-sm font-medium rounded bg-primary/10 text-primary line-clamp-2 text-left">
                                        {product.productFamily}
                                    </span>
                                </td>
                                <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white " style={{ width: widths.unitPrice }}>
                                    ${product.unitPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                                <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white" style={{ width: widths.quantity }}>{product.quantity.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white font-semibold" style={{ width: widths.subtotal }}>
                                    ${product.subtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                                <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white" style={{ width: widths.shipping }}>
                                    ${product.shipping.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                                <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white" style={{ width: widths.taxes }}>
                                    ${product.taxes.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                                <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white font-semibold" style={{ width: widths.grandTotal }}>
                                    ${product.grandTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                                <td className="px-4 py-3 text-center" style={{ width: widths.actions || 100 }}>
