...
export default function ShipmentFilesTab({ shipmentId, accountId, contactId, isEditing = false, onFilesCountChange }: ShipmentFilesTabProps) {
    ...
    const fetchFiles = async () => {
        try {
            setLoading(true);
            const res = await fetch(`/api/salesforce/shipments?action=files&accountId=${encodeURIComponent(accountId)}&contactId=${encodeURIComponent(contactId)}&objectId=${encodeURIComponent(shipmentId)}`);
    ...
    useEffect(() => {
        if (shipmentId && accountId && contactId) {
            fetchFiles();
        }
    ...
    const handleDownload = async (file: FileData) => {
        ...
        try {
            ...
            const res = await fetch(
                `/api/salesforce/shipments?action=download&accountId=${encodeURIComponent(accountId)}&contactId=${encodeURIComponent(contactId)}&objectId=${encodeURIComponent(shipmentId)}&contentVersionId=${encodeURIComponent(contentVersionId)}`
            );
