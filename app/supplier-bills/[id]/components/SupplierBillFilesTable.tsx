    ...
    if (files.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm">There are no files attached to this supplier bill.</p>
            </div>
        );
    }
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {paginatedData.map((file) => (
                            <tr key={file.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                ...
                                <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400" title={file.fileType}>{file.fileType}</td>
                                <td className="px-3 py-2 text-sm text-gray-900 dark:text-white" title={formatFileSize(file.sizeInBytes)}>{formatFileSize(file.sizeInBytes)}</td>
                                <td className="px-3 py-2 text-sm text-gray-900 dark:text-white"><div className="truncate" title={file.uploadedBy}>{file.uploadedBy}</div></td>
                                <td className="px-3 py-2 text-sm text-gray-500 dark:text-gray-400" title={file.uploadedDate ? formatDate(file.uploadedDate) : '-'}>{file.uploadedDate ? formatDate(file.uploadedDate) : '-'}</td>
