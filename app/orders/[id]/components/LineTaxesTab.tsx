...
export default function LineTaxesTab({ product, loading }: LineTaxesTabProps) {
    ...
    if (!product) {
        return (
            <TableEmptyState message="No tax information available." />
        );
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden p-4">
            ...
            <div className="overflow-x-auto">
                <Table className="table-fixed">
                        ...
                        <TBody>
                            <Tr>
                                <Td className="text-left truncate" style={{ width: widths.salesTaxRate, minWidth: widths.salesTaxRate, maxWidth: widths.salesTaxRate }} title={formatPercent(product.Sales_Tax_Rate__c)}>
                                    {formatPercent(product.Sales_Tax_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.salesTaxAmount, minWidth: widths.salesTaxAmount, maxWidth: widths.salesTaxAmount }} title={formatTax(product.Sales_Tax_Amount__c)}>
                                    {formatTax(product.Sales_Tax_Amount__c)}
                                </Td>

                                <Td className="text-left truncate" style={{ width: widths.useTaxRate, minWidth: widths.useTaxRate, maxWidth: widths.useTaxRate }} title={formatPercent(product.Use_Tax_Rate__c)}>
                                    {formatPercent(product.Use_Tax_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.useTaxAmount, minWidth: widths.useTaxAmount, maxWidth: widths.useTaxAmount }} title={formatTax(product.Use_Tax_Amount__c)}>
                                    {formatTax(product.Use_Tax_Amount__c)}
                                </Td>

                                <Td className="text-left truncate" style={{ width: widths.localTaxRate, minWidth: widths.localTaxRate, maxWidth: widths.localTaxRate }} title={formatPercent(product.Local_Tax_Rate__c)}>
                                    {formatPercent(product.Local_Tax_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.localTaxAmount, minWidth: widths.localTaxAmount, maxWidth: widths.localTaxAmount }} title={formatTax(product.Local_Tax_Amount__c)}>
                                    {formatTax(product.Local_Tax_Amount__c)}
                                </Td>

                                <Td className="text-left truncate" style={{ width: widths.exciseTaxRate, minWidth: widths.exciseTaxRate, maxWidth: widths.exciseTaxRate }} title={formatPercent(product.Excise_Tax_Rate__c)}>
                                    {formatPercent(product.Excise_Tax_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.exciseTaxAmount, minWidth: widths.exciseTaxAmount, maxWidth: widths.exciseTaxAmount }} title={formatTax(product.Excise_Tax_Amount__c)}>
                                    {formatTax(product.Excise_Tax_Amount__c)}
                                </Td>

                                <Td className="text-left truncate" style={{ width: widths.grtRate, minWidth: widths.grtRate, maxWidth: widths.grtRate }} title={formatPercent(product.Gross_Receipts_Tax_Rate__c)}>
                                    {formatPercent(product.Gross_Receipts_Tax_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.grtAmount, minWidth: widths.grtAmount, maxWidth: widths.grtAmount }} title={formatTax(product.Gross_Receipts_Tax_Amount__c)}>
                                    {formatTax(product.Gross_Receipts_Tax_Amount__c)}
                                </Td>

                                <Td className="text-left truncate" style={{ width: widths.gstRate, minWidth: widths.gstRate, maxWidth: widths.gstRate }} title={formatPercent(product.GST_Rate__c)}>
                                    {formatPercent(product.GST_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.gstAmount, minWidth: widths.gstAmount, maxWidth: widths.gstAmount }} title={formatTax(product.GST_Amount__c)}>
                                    {formatTax(product.GST_Amount__c)}
                                </Td>

                                <Td className="text-left truncate" style={{ width: widths.vatRate, minWidth: widths.vatRate, maxWidth: widths.vatRate }} title={formatPercent(product.VAT_Rate__c)}>
                                    {formatPercent(product.VAT_Rate__c)}
                                </Td>
                                <Td className="text-left truncate" style={{ width: widths.vatAmount, minWidth: widths.vatAmount, maxWidth: widths.vatAmount }} title={formatTax(product.Total_VAT_Amount__c)}>
