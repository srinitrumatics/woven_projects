...
- [ ] T010 [US2] Using live Salesforce test credentials and a product known to have at least one photo, call the existing product-details path (`/api/salesforce/product-details?accountId=...&contactId=...&productId=...&tabName=product`) and inspect the raw response to confirm `gtherp__Image_URL__c` is actually present — the field name is confirmed from the org owner, but whether the Apex REST endpoint's response allowlist already includes it has **not** been separately verified. Not run this session — needs live SF access.
- [ ] T011 [US2] If T010 finds the Apex endpoint `gtherp/product/details` does not return `gtherp__Image_URL__c` in its response, coordinate with the Salesforce org admin to add it (an Apex-side change, outside this repository) — per the dependency flagged in `contracts/product-detail-images.md`. Depends on T010.
